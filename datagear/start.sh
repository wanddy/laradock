...
APP_CLASSPATH=.:lib/datagear-webappembd-1.10.1.jar:lib/javax.servlet-3.0.0.v201112011016.jar:lib/jetty-all-8.1.22.v20160922.jar:lib/slf4j-api-1.7.9.jar:lib/slf4j-log4j12-1.7.9.jar:lib/log4j-1.2.17.jar 
...
if [ $APP_PID -ne 0 ]; then 
    ...
    if [ -n "$JAVA_HOME" ]; then 
        $JAVA_HOME/bin/java $JAVA_OPTS -cp $APP_CLASSPATH $APP_MAIN
    else 
        java $JAVA_OPTS -cp $APP_CLASSPATH $APP_MAIN
    fi 
